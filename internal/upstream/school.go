...
// ---- growth 域 mp 成长任务续环判据载体（Sequential_Tasks_2..7）----

// MiniExpertUseEvent 构造 growth 域 Sequential_Tasks_2「在小程序内选中专家并完成
// 有效对话」的判据事件：mp 指纹 expert_actual_use。形状对齐小程序源码
// app-service.js 的真实发射点（上游 task_runner 实测：上报即 completed，claim
// +200c+5e）。与 school 域的 SchoolExpertUseEvents 是**两套口径**，勿照抄：
...
//
// expertID 必须是专家市场真实 ex_ id（Client.MarketExpertList），空 id 服务端不入账。
...
// 载体）：小程序 chat_request_send 真实发射点带 requestModelId/requestModelName——
// Tasks_1/3 的裸对话事件不带模型，模型任务须用本形态（判据待解锁实测验证）。
...
// MiniPlaybookEvents mp 指纹灵感事件组（Sequential_Tasks_7「体验灵感功能」判据载体，
// 形状对齐 mpsrc 发射点：playbook_cta_click → playbook_prompt_send）。该任务疑为
// PC 口径（+500c+5e）——PC 序列（DesktopPlaybookPromptSequence）已实测点亮
// playbook_prompt，本组作为 mp 形态补充（任务在 mp 链上，判据究竟认哪侧待解锁实测）。

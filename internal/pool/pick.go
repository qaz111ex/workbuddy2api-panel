...
func (p *Pool) pick(tried map[string]bool, reqModel, realm string) *auth.Auth {
	...
	for uid, e := range p.byUID {
		...
		e.pruneExpiredModelCosts(now)     // 成本台账同理（只增不减会随历史模型名累积）
		if !healthyOf(e) {
			continue
		}
		if p.inFlightFull(e) {
	...
	// 条件探索（issue #136 方案 a′）：tier 0 垄断层存在（bestTier==0 且 reqModel 非空）
	// 且候选含 tier 1（冻结存在）且距上次探索 ≥ 窗口（零值 timer=从未探索→首次满足即探）
	// 时，本次 pick 生效层切 tier 1-only——探索=搭车改道，把一个既有真实用户请求改道给
	// 未知号（零新增上游请求；IP 维度零增量，WAF 友好）。成功 → NoteModelCost 首观测 →
	// 毕业（tier 0/2，下一轮 pick 立即生效）；失败 → 既有错误策略照常，无探测风暴。
	// timer 同锁写入：并发 pick 串行进入写锁，只有一个进入者能通过窗口判定（天然防重复）。
	// key = realm + "\x1f" + reqModel：同模型名可跨域，探索节奏按 (域, 模型) 独立。
	...
	if explored {
		...
		// 毕业结果由相邻的既有日志闭环（免费号无日志、收费号走 NoteModelCost 常规路径）。
...
func (p *Pool) pickEarliestExpiryLocked(tried map[string]bool, now time.Time, realm string) *auth.Auth {
	...
	// 兜底同样是「选中」，必须与 pick() 正常路径一样推进 usedSeq/pickSeq：否则被
	// 兜底反复选中的账号 usedSeq 恒为 0，在 pick 的 LRU 兜底（按 usedSeq 取最旧）
	// 眼里永远是「最旧」，刚被用过就被立刻再选——防集中/防惊群失效
	// （entry.usedSeq 契约：每次被选中时取 p.pickSeq 自增值）。
	p.pickSeq++

...
func TestNoteModelCostDeductsExpiring(t *testing.T) {
	p := New("")
	p.Add(&auth.Auth{UID: "u1"})
	p.SetCreditsDetailed("u1", 100, 0, 50) // 50 快过期（本 fork 签名：credits, total, expiring）
...
func TestNoteModelCostExpiringClampsAtZero(t *testing.T) {
	p := New("")
	p.Add(&auth.Auth{UID: "u1"})
	p.SetCreditsDetailed("u1", 100, 0, 5)
...
func TestWeightOfCalledOncePerPick(t *testing.T) {
	...
	p.SetCredits("u1", 100, 100)
	p.SetCredits("u2", 50, 50)
	a := p.PickExcludingForModel(nil, "")
	if a == nil {
...
func TestPickWeightedUsesFullSetMaxCredits(t *testing.T) {
	...
	got := p.PickExcludingForModel(nil, "")
...
func TestWeightOfMaxCreditsPassedVerbatim(t *testing.T) {
	...
	p.SetCredits("u1", 1000, 1000)
	p.SetCredits("u2", 500, 500)
	p.SetCredits("u3", 400, 400)
	if got := p.PickExcludingForModel(nil, ""); got == nil {
		t.Fatal("pick returned nil")
	}
	if seen[1000] == 0 {
...
func TestWeightOfThreeFactorsNoSuccess(t *testing.T) {
	...
	p.SetCredits("good", 100, 100)
	p.SetCredits("bad", 100, 100)
	for i := 0; i < 20; i++ {
		p.NoteSuccess("good")
		p.NoteError("bad")
	}
...
func TestShuffleEpsilonEqualWeights(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		p.SetCredits(uid, 100, 100) // 完全同构 → 等权重
	}

...
func TestNoteModelCostAndPreferFree(t *testing.T) {
	...
	p.SetCredits("free", 1, 1)
	p.SetCredits("paid", 1_000_000, 1_000_000)
...
func TestModelCostFreeBeatsUnknown(t *testing.T) {
	...
	p.Add(&auth.Auth{UID: "unknown"})
	p.SetCredits("knownfree", 1, 1)
	p.SetCredits("unknown", 1_000_000, 1_000_000)
...
func TestModelCostPrunedOnPick(t *testing.T) {
	...
	p.PickExcludingForModel(nil, "") // pick 写锁路径做惰性回收

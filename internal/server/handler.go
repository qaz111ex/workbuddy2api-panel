...
type Config struct {
	...
	// 请求体大小上限已**彻底移除**（原 MaxBodyBytes 字段与 SetMaxBodyBytes 热更
	// setter 一并删除）：网关不再做字节级预拦截，请求体完整读入并转发上游，超限
	// 类问题由上游按其真实规则答复（上游限制在 token，见 cmd/server/config.go 的
	// 说明）。读 body 出错（客户端中途断流）仍就地 400——issue #41 的截断防御语义
	// 保留在读错误路径，不把半截 JSON 喂上游 unmarshal 冤枉罚号。
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	// 请求体读取：**无大小上限**（server.max_body_mb 已彻底移除，对齐上游
	// e34cfa4/a0aae43）：完整读入并转发，超限类问题交由上游按其真实规则答复
	// （其限制在 token 而非字节，响应信息量比网关的 413 更大）。
	// 读 body 出错（客户端中途断流）就地 400——#41 的截断防御语义保留在读错误
	// 路径：不把半截 JSON 喂上游 unmarshal 报 unexpected EOF，反而冤枉罚号。

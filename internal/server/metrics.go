...
//
// 与 fork 的适配点（本 fork 与 sk 上游形态不同，务必注意）：
//   - chatStat 的 token 字段是 promptTokens/completionTokens/totalTokens 三件套 +
//     hasXxx 三闸（上游只有 hasUsage/tokens/prompt）；本文件只读 prompt/toks 两个
//     已归一的聚合字段，并额外用 hasUsage（由 UsageSeen() 填充）。
//   - chatStat.Usage()（pool.TokenUsageDelta）是 fork 独有、服务 /panel/api/usage
//     用量时间线的方法，**不得删除**；本文件与它无关，只是共用同一个 chatStat。
package server
...
func recordChatMetric(s *chatStat, total time.Duration) {
	...
	// token / cache 只在 hasUsage 时累加：缺失≠0。hasUsage 由
	// chatStatsReader.UsageSeen() 填充（任一 token 字段出现即算有观测），
	// 因此「末帧只带 prompt_tokens」的部分观测也能被计入，不会被整段漏掉。
	if s.hasUsage {
		mm.promptTok += int64(s.prompt)
		// toks<0 是「观测缺失」哨兵（非流式路径：usage 存在但缺 completion_tokens 时
		// completionTokens 返回 -1）。不设此防护会把 -1 累加进总量，越积越偏——
		// 真值只可能 ≥0，故负值一律不计。
		if s.toks > 0 {
...
//
// 注意本 fork 的 chatStat 用 toks/prompt 两个归一字端：
//   - hasUsage 只要有 usage 对象即为真（非流式 usage 是整帧给全的，不存在「只有
//     prompt 没有 completion」的部分观测形态）；
//   - toks 不在此写入：调用方已用 completionTokens() 落值（缺 completion_tokens
//     时是 -1 哨兵），metrics 侧对负值不计入——与既有的 "-" 展示口径一致。

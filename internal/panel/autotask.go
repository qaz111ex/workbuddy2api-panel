...
var mpTaskCodes = map[string]bool{
	...
	// found 封顶）。链条每日零点解锁一环（task locked until 次日），判据为上游 issue
	// 描述 + mpsrc 事件形状预置，解锁后逐个实测校正。
	"Sequential_Tasks_4": true, // 小程序定时任务（判据疑 PC 口径 automation）
	"Sequential_Tasks_5": true, // 小程序使用 GLM5.2（mini chat + 模型字段）
	"Sequential_Tasks_6": true, // 小程序十次对话（target 差额补报）
	"Sequential_Tasks_7": true, // 体验灵感功能（PC 灵感组 primary + mp 形态 fallback）
}
...
// 若统一兜底为 1，target=5/10 的多元任务只会补报 1 次。故兜底必须按**任务规格**取：
// 表内命中用表值，未登记才回落 1。规格值随上游解锁/校正时同步更新本表。
var mpTaskSpecTarget = map[string]int64{
	"Sequential_Tasks_1": 1,  // 小程序首对话
	"Sequential_Tasks_2": 1,  // 选中专家并完成有效对话
	"Sequential_Tasks_3": 5,  // 5 次有效对话（上游 issue 实测）
	"Sequential_Tasks_4": 1,  // 定时任务（预留）
	"Sequential_Tasks_5": 1,  // 使用 GLM5.2（预留）
	"Sequential_Tasks_6": 10, // 10 次对话（预留，上游 issue 称 target=10）
	"Sequential_Tasks_7": 1,  // 体验灵感功能（预留）
}

// mpTargetFallback 返回任务规格兜底 target：任务自报 target 优先，其次规格表，最后 1。
func mpTargetFallback(code string, reported int64) int64 {
	if reported > 0 {
		return reported
	}
	if spec, ok := mpTaskSpecTarget[code]; ok && spec > 0 {
		return spec
	}
	return 1
}
...
// mp 查询 → accept（带登记回读验证）→ **accept 后回读**（取服务端权威 target/current）
// → target 兜底 → mini chat 事件上报（withActivityId 决定是否带开学季 activityId：
// school_season 必带，Sequential_Tasks_1/3/6 不带——服务端按 source=mini_program
// 指纹关联）→ 回读 → 达标即领奖。
//
// 顺序要点（勿回退）：
//   - mp 任务**未 accept 时 progress 为 null**（target 读成 0）。若在 accept 之前就用
//     兜底 target 判达标，target=5 的多元任务会被判成「已达标」而一次都不上报。
...
func (p *Panel) runMPMiniChatTask(a *auth.Auth, code string, withActivityId bool) (string, error) {
	...
	if t.AcceptStatus == "not_accepted" || t.AcceptStatus == "" {
		...
		// accept 后回读：未接受时 progress 为 null（target=0），接受后才是权威规格。
		if t2, err2 := p.taskByCodeMP(a, code); err2 == nil && t2 != nil {
			t = t2
		}
	}
	// 已达标（含 completed 未领）：直接领奖。
	target := mpTargetFallback(code, t.Target)
...
// 列表：拉不到就整任务不动作，避免留下「已登记未上报」的半程态。
// 复用既有 MarketExpertList（expert_5 任务同源，实测可用）。
func runMiniExpert(p *Panel, a *auth.Auth) (string, error) {
	...
	// 判据载体前置（accept 之前）：市场真实专家 id。拿不到就整任务不动作，
	// 不留「已登记未上报」的半程态（上游 task_runner ids 前置判定同款）。
...
// runSequentialEventTask Sequential 链预留任务（Tasks_4/5/7）通用骨架：
// mp 查询 → accept（带登记回读验证）→ 判据事件上报（primary；未点亮且有 fallback
// 时补报一轮）→ 回读 → 达标领奖。每日零点解锁一环：locked 期间 accept 不落账
// （上游 accept 返回 HTTP 200 + code 0，per-task error 藏在 data.results[] 里，
// 回读验证是唯一暴露手段——acceptWithVerifyMP 天然扛住），返回等下次调度即可。
...
// mp 源码无 automation 事件发射点 → 判据疑为 PC 口径：复用 automation_1 同源事件
// （DesktopAutomationCreateEvent，PC 任务三账号实测点亮）。
...
// primary：mp 对话事件带 requestModelId/requestModelName=glm-5.2（mpsrc 发射点实测
// 形状）；fallback：PC 域模型活跃上报（Model_chat_GLM5.2 同源）。
...
// primary：PC 灵感事件组（DesktopPlaybookPromptSequence，playbook_prompt 三账号实测
// 点亮）；fallback：mp 指纹灵感事件组（MiniPlaybookEvents，mpsrc 形状）。

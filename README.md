...
| 分类 | 触发条件 | 账号处置 | 恢复 |
|---|---|---|---|
| 余额不足 | HTTP 402 / body 含余额关键词 / **HTTP 429 + code `14018`** | 硬冷却到**次日 04:00**（本地时区） | 签到（09/21 点）余额恢复自动解冻 |
...
| 图片请求无效 | HTTP 400 + code `11135` / `invalid_image_data` / `invalid image_url content` / `replace the image` | **不罚账号、不轮转**：图片格式/数据错误是确定性请求级错误，同一 body 换任何账号结果不变，直接透传上游原文（`code: image_invalid`）。避免把健康账号白转一圈后仍返回失败 | 修正请求里的图片即可 |
| 上下文超限 | HTTP 400/404 + code `11115` / `prompt is too long` | **不罚账号、不轮转**：请求级错误，透传上游原文（含真实 token 数与上限） | 缩减上下文 |
...
> **`429` + `code 14018` 单独列出**：它是明确的**账号积分耗尽**，却被上游以 429 状态码返回。若按通用 429 当软限流处理，该号会被判为「可自愈」并在全池冷却时被反复兜底选中、反复失败；因此它按结构化业务码单独归入硬冷却。无该 code 的 `credits exhausted` 文案仍按普通 429 的软限流语义处理（不靠跨计费/限流两界的文案猜测）。
>
> **图片与上下文的 fail-fast**：这两类是**确定性的请求级错误**（同一 body 换任何账号结果都一样），故直接透传不轮转——否则无效请求会被放大成整池各试一遍，最终仍返回失败。客户端可据 `error.code`（`image_invalid` / `prompt_too_long`）与 `gateway_hint` 定位问题。
